...
import { ArrowRight, ShieldCheck, Truck, Award, Factory, Recycle, Headphones, CheckCircle2, Star, Quote } from "lucide-react";
...
const why = [
  ...
  { icon: ShieldCheck, t: "Consistent Quality", d: "ISO-aligned processes with in-house inspection ensure every batch meets specification." },
  { icon: Factory, t: "In-House Manufacturing", d: "Complete control over production, slitting and finishing for reliable, repeatable output." },
  { icon: Truck, t: "On-Time Delivery", d: "A responsive supply chain that keeps your dispatch and production lines running." },
  { icon: Recycle, t: "Custom & OEM Solutions", d: "Bespoke widths, colours, printing and die-cut profiles tailored to your needs." },
  { icon: Headphones, t: "Dedicated Support", d: "Technical guidance and prompt service from enquiry to after-sales." },
];

const process = [
  { t: "Raw Material Sourcing", d: "Premium films, foams and adhesives sourced from trusted suppliers." },
  { t: "Coating & Extrusion", d: "Precision adhesive coating and foam extrusion to exact specifications." },
  { t: "Slitting & Cutting", d: "Accurate slitting, die-cutting and CNC routing to required sizes." },
  { t: "Quality Inspection", d: "In-process and final testing for tack, thickness and performance." },
  { t: "Packing & Dispatch", d: "Careful packing and timely dispatch across India and for export." },
];

const testimonials = [
  { n: "Rajesh Mehta", c: "Procurement Head, FMCG", t: "Vardhman's BOPP tapes hold up perfectly on our high-speed lines. Consistent quality and reliable delivery every single time." },
  { n: "Anita Sharma", c: "Operations, E-commerce", t: "Their EPE foam profiles cut our transit damage significantly. The custom C-sections fit our products exactly." },
  { n: "Suresh Patel", c: "Plant Manager, Automotive", t: "The panel bonding and PE foam tapes perform brilliantly under heat and vibration. A dependable long-term partner." },
];
    ...
    <>
        ...
        <div className="container-x grid items-center gap-12 lg:grid-cols-2">
            ...
            <div className="grid grid-cols-2 gap-4">
              ...
              <div className="grid gap-4">
                ...
                <div className="flex flex-col justify-center rounded-2xl bg-brand-gradient p-6 text-white">
                  <div className="font-display text-4xl font-extrabold"><Counter to={60} suffix="+" /></div>
          ...
          <div>
            ...
            <Reveal delay={0.15}>
              ...
              <Link href="/about" className="btn-primary mt-8">Learn More About Us <ArrowRight className="h-4 w-4" /></Link>
            </Reveal>
          </div>
        </div>
      </section>
      ...
      <section className="section bg-brand-gray">
        <div className="container-x">
          <SectionHeading center eyebrow="Why Vardhman" title="Why businesses choose us" subtitle="Six reasons companies across India rely on Vardhman Packaging for their tape, foam and packaging needs." />
            ...
            {why.map((w, i) => (
              <Reveal key={w.t} delay={i * 0.05}>
                <div className="card group h-full p-6 transition hover:-translate-y-1 hover:shadow-glow">
                  <div className="flex h-12 w-12 items-center justify-center rounded-xl bg-brand-yellow/15 text-brand-orange transition group-hover:bg-brand-gradient group-hover:text-white"><w.icon className="h-6 w-6" /></div>
                  <h3 className="mt-4 font-display text-lg font-bold text-brand-dark">{w.t}</h3>
            ...
            {industries.map((ind, i) => (
              <Reveal key={ind.slug} delay={i * 0.03}>
                <Link href="/industries" className="card group flex flex-col items-center gap-3 p-5 text-center transition hover:-translate-y-1 hover:shadow-card">
                  <div className="flex h-12 w-12 items-center justify-center rounded-full bg-brand-gray text-brand-orange transition group-hover:bg-brand-gradient group-hover:text-white"><Icon name={ind.icon} className="h-6 w-6" /></div>
      ...
      <section className="section bg-brand-gray">
        <div className="container-x">
          <div className="flex flex-col items-start justify-between gap-4 sm:flex-row sm:items-end">
            ...
            <Link href="/products" className="btn-outline flex-none">View all products <ArrowRight className="h-4 w-4" /></Link>
          </div>
          <div className="mt-12 grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
            {featured.map((p, i) => {
              const cat = categories.find((c) => c.slug === p.category)!;
              return <Reveal key={p.slug} delay={i * 0.05}><ProductCard p={p} category={cat} /></Reveal>;
            })}
          </div>
        </div>
      </section>
            ...
            {process.map((p, i) => (
              <Reveal key={p.t} delay={i * 0.06} className="relative">
                <div className="flex flex-col items-center text-center">
                  <div className="relative flex h-14 w-14 items-center justify-center rounded-2xl bg-brand-gradient font-display text-xl font-bold text-white shadow-soft">{i + 1}</div>
                  <h3 className="mt-4 font-display text-base font-bold text-brand-dark">{p.t}</h3>
                  <p className="mt-2 text-sm text-black/55">{p.d}</p>
                </div>
                {i < process.length - 1 && <div className="absolute right-[-1rem] top-7 hidden h-0.5 w-8 bg-gradient-to-r from-brand-orange/40 to-transparent md:block" />}
              </Reveal>
            ))}
          </div>
        </div>
      ...
      <section className="section bg-brand-dark text-white">
        <div className="container-x grid items-center gap-12 lg:grid-cols-2">
          <div>
            ...
            <p className="mt-4 text-white/70 leading-relaxed">Quality is engineered into every stage of our process. From incoming raw-material checks to in-process monitoring and final testing, our team ensures tack, thickness, adhesion and dimensional accuracy meet specification before dispatch.</p>
            <div className="mt-8 grid gap-4 sm:grid-cols-2">
              {["Raw material inspection", "In-process monitoring", "Adhesion & tack testing", "Dimensional accuracy checks"].map((q) => (
                <div key={q} className="flex items-center gap-3 rounded-xl bg-white/5 px-4 py-3"><CheckCircle2 className="h-5 w-5 flex-none text-brand-yellow" /><span className="text-sm text-white/80">{q}</span></div>
              ))}
            </div>
            <Link href="/quality" className="btn bg-brand-gradient mt-8 text-white">Our Quality Process <ArrowRight className="h-4 w-4" /></Link>
          </div>
          <Reveal className="grid grid-cols-2 gap-4">
            {company.stats.map((s) => (
              <div key={s.label} className="rounded-2xl border border-white/10 bg-white/5 p-6 text-center">
                <div className="font-display text-4xl font-extrabold text-brand-yellow"><Counter to={s.value} suffix={s.suffix} /></div>
            ...
            {testimonials.map((t, i) => (
              <Reveal key={t.n} delay={i * 0.06}>
                <div className="card h-full p-7">
                  ...
                  <p className="mt-4 text-sm leading-relaxed text-black/70">{t.t}</p>
                  <div className="mt-5 flex items-center gap-1 text-brand-yellow">{[...Array(5)].map((_, k) => <Star key={k} className="h-4 w-4" fill="currentColor" />)}</div>
                  <div className="mt-4 border-t border-black/5 pt-4">
                    <div className="font-display font-bold text-brand-dark">{t.n}</div>
      ...
      <section className="section">
        <div className="container-x">
          <div className="flex flex-col items-start justify-between gap-4 sm:flex-row sm:items-end">
            ...
            <Link href="/blogs" className="btn-outline flex-none">All articles <ArrowRight className="h-4 w-4" /></Link>
          </div>
          <div className="mt-12 grid gap-6 md:grid-cols-3">
            {blogs.slice(0, 3).map((b, i) => (
              <Reveal key={b.slug} delay={i * 0.06}>
                <Link href={`/blogs/${b.slug}`} className="group card overflow-hidden transition hover:-translate-y-1 hover:shadow-glow">
                  ...
                  <div className="p-5">
                    <div className="flex items-center gap-3 text-xs text-black/45"><span className="chip">{b.category}</span><span>{b.readTime}</span></div>
                    <h3 className="mt-3 font-display font-bold text-brand-dark group-hover:text-brand-orange line-clamp-2">{b.title}</h3>
      ...
      <section className="section bg-brand-gray">
        <div className="container-x grid gap-12 lg:grid-cols-2">
          ...
          <div>
            ...
            <div className="mt-8"><EnquiryForm compact /></div>
          </div>
        </div>
      </section>
