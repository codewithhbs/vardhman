...
export const metadata = {
  title: "Products",
  description:
    "Explore our full range of industrial tapes, BOPP packaging tapes, double-sided & electrical tapes, EPE/EVA foam, backer rods, LDPE sheets and specialty products.",
};
...
export default async function Products() {
  ...
  return (
    <>
      <PageHero
        title="Our Products"
        subtitle="A complete range of industrial tapes, foam profiles and packaging solutions — organised by category, customisable to your specification."
            ...
            <Reveal key={c.slug} delay={(i % 3) * 0.05}>
              <Link href={`/products/${c.slug}`} className="group card overflow-hidden transition hover:-translate-y-1 hover:shadow-glow">
                <div className="relative aspect-[16/10] overflow-hidden">
                  ...
                  <div className="absolute left-4 top-4 flex h-11 w-11 items-center justify-center rounded-xl bg-white/90 text-brand-orange backdrop-blur">
                    <Icon name={c.icon} className="h-6 w-6" />
                  </div>
                  <span className="absolute bottom-4 right-4 rounded-full bg-white/90 px-3 py-1 text-xs font-semibold text-brand-dark backdrop-blur">
