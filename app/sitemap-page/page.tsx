        ...
        <div className="container-x grid gap-10 md:grid-cols-3">
          <div>
            ...
            <ul className="mt-4 space-y-2">
              {pages.map(([l, h]) => (
                <li key={h}><Link href={h} className="text-sm text-black/60 hover:text-brand-orange">{l}</Link></li>
              ))}
            </ul>
          </div>
          <div>
            ...
            <ul className="mt-4 space-y-2">
              {categories.map((c) => (
                <li key={c.slug}><Link href={`/products/${c.slug}`} className="text-sm text-black/60 hover:text-brand-orange">{c.name}</Link></li>
              ))}
            </ul>
          </div>
          <div>
            ...
            <ul className="mt-4 space-y-2">
              {blogs.map((b) => (
                <li key={b.slug}><Link href={`/blogs/${b.slug}`} className="text-sm text-black/60 hover:text-brand-orange">{b.title}</Link></li>
              ))}
            </ul>
          </div>
        </div>
